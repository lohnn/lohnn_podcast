...
import 'package:podcast_core/data/user_episode_status.model.dart'; // For EpisodeWithStatus
...
import 'package:audio_service/audio_service.dart'; // For default PlaybackState
import '../../../test_data_models/test_episode.dart' as test_data; // Aliased to avoid name clash
...
void main() {
  ...
  Future<void> pumpWidgetUnderTest(
    WidgetTester tester, {
    required AsyncValue<EpisodeWithStatus?> currentEpisodeWithStatusProviderValue,
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          currentEpisodeWithStatusPodProvider.overrideWith((ref) => currentEpisodeWithStatusProviderValue),
          // Provide a default for audioStateProvider for the PlayPauseButton
          audioStateProvider.overrideWith((ref) => AsyncData(audioState ?? createDefaultPlaybackState())),
        ],
  ...
  group('SmallMediaPlayerControls Tests', () {
    testWidgets('renders correctly when playing an episode', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        ...
        expect(find.byType(InkWell), findsOneWidget); // The whole control is tappable
        ...
        expect(find.byType(DownloadAnimation), findsOneWidget); // Assuming it's always there
        expect(find.byType(MediaActionButton), findsNothing); // showSkipButtons is false
      });
    });

    testWidgets('renders correctly with skip buttons', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        await pumpWidgetUnderTest(
          tester,
          currentEpisodeWithStatusProviderValue: AsyncData(mockEpisodeData),
    ...
    testWidgets('renders error state', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        await pumpWidgetUnderTest(
          tester,
          currentEpisodeWithStatusProviderValue: AsyncError('Test Error', StackTrace.empty),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets('renders nothing playing state', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        await pumpWidgetUnderTest(
          tester,
          currentEpisodeWithStatusProviderValue: const AsyncData(null), // Null episode data
        );
        await tester.pumpAndSettle();
