...
void main() {
  ...
  final mockPodcast = testPodcast.copyWith(
    title: 'Accessible Podcast Title', // Using a distinct title for clarity
    episodes: [],
  );
  final podcastId = mockPodcast.id;

  final mockEpisode = testEpisode.copyWith(podcastId: podcastId, title: 'Test Episode 1 for Details');
  ...
  group('PodcastDetailsScreen Tests', () {
    testWidgets('renders correctly with data and checks key semantics', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(
          createTestApp(
            PodcastDetailsScreen(podcastId: podcastId),
            [
              episodesProvider(podcastId: podcastId).overrideWith(
                (ref) => Stream.value(AsyncData((mockPodcast, [mockEpisodeWithStatus]))),
              ),
              episodesFilterProvider.overrideWith((ref) => EpisodesFilterState()),
            ],
          ),
        );
        ...
        expect(tester.getSemantics(filterButton), matchesSemantics(
            hasTapAction: true,
            isButton: true,
        ...
        expect(find.descendant(of: find.byType(AppBar), matching: appBarTitleFinder), findsOneWidget);
        final SemanticsNode appBarTitleSemantics = tester.getSemantics(appBarTitleFinder);
        // Titles in AppBar are often marked as headers by default
        expect(appBarTitleSemantics.hasFlag(SemanticsFlag.isHeader), isTrue, reason: "AppBar title should be a header");
      ...
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(
          createTestApp(
            ...
            [
              episodesProvider(podcastId: podcastId).overrideWith(
                (ref) => const Stream.value(AsyncLoading()),
              ),
              episodesFilterProvider.overrideWith((ref) => EpisodesFilterState()),
            ],
          ),
      ...
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(
          createTestApp(
            ...
            [
              episodesProvider(podcastId: podcastId).overrideWith(
                (ref) => Stream.value(AsyncError('Test error', StackTrace.empty)),
              ),
              episodesFilterProvider.overrideWith((ref) => EpisodesFilterState()),
            ],
          ),
    ...
    testWidgets('renders correctly with empty episode list', (WidgetTester tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(
          createTestApp(
            PodcastDetailsScreen(podcastId: podcastId),
            [
              episodesProvider(podcastId: podcastId).overrideWith(
                (ref) => Stream.value(AsyncData((mockPodcast, []))),
              ),
              episodesFilterProvider.overrideWith((ref) => EpisodesFilterState()),
            ],
          ),
